...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		...
		align    = 'top-left',
		elements = {
			{label = 'LSMC Menu', value = 'citizen_interaction'},
			{label = 'Rechnung', value = 'billing'}
			--{label = 'Objekt Spawner', value = 'object_spawner'}
		}
	}, function(data, menu)
		if data.current.value == 'billing' then
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
                title = ('Höhe der Rechnung')
            }, function(data, menu)
                local amount = tonumber(data.value)
        
                if amount == nil or amount < 0 then
                    TriggerEvent('scarface_notify', '#ff0000', 'Ungültige Menge')
                else
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        TriggerEvent('scarface_notify', '#ff0000', 'Kein Spieler in der Nähe')
                    else
                        menu.close()
                        TriggerServerEvent('esx_nwdoanefleanmfeoifnaofebngoengao:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'LSMC', amount)
                    end
                end
            end, function(data, menu)
                menu.close()
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				align    = 'top-left',
				elements = {
					{label = 'Unbenannt', model = 'p_ld_heist_bag_01'},
					{label = 'Unbenannt', model = 'prop_ld_binbag_01'},
					{label = 'Unbenannt', model = 'prop_cs_shopping_bag'},
					{label = 'Unbenannt', model = 'prop_ld_binbag_03'},
					{label = 'Unbenannt', model = 'prop_ld_binbag_04'},
					{label = 'Unbenannt', model = 'prop_ld_case_01'},
					{label = 'Unbenannt', model = 'prop_med_bag_01'},
					{label = 'Unbenannt', model = 'prop_med_bag_01b'}
		...
		elseif data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				align    = 'top-left',
				elements = {
					{label = 'Wiederbelebe Person', value = 'revive'},
					{label = 'Heile kleine Wunden', value = 'small'},
					{label = 'Heile große Wunden', value = 'big'},
					{label = 'Setze Person in Fahrzeug', value = 'put_in_vehicle'}
				}
			}, function(data, menu)
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if IsPedDeadOrDying(closestPlayerPed, 1) then
									...
									TriggerServerEvent('scarface_fickdeinemutterduskid:wafanfwopfwfpw', GetPlayerServerId(closestPlayer))
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('scarface_fickdeinemuttedwwadwdawdadwadwwrduskid:wafanfwopfdwadwdawwfpw', GetPlayerServerId(closestPlayer), 'small')
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('esx_ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('scarface_fickdeinemuttedwwadwdawdadwadwwrduskid:wafanfwopfdwadwdawwfpw', GetPlayerServerId(closestPlayer), 'big')
									ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)))
									IsBusy = false
								else
									ESX.ShowNotification(_U('player_not_conscious'))
...
Citizen.CreateThread(function()
	while true do
		...
		elseif ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'ambulance' and not IsPlayerDead(source) then
...
function OpenVehicleSpawnerMenu(hospital, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		elseif data.current.action == 'garage' then
			...
			ESX.TriggerServerCallback('wdadgeagteafacetefaxc:retrieveJobVehicles', function(jobVehicles)
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									...
									TriggerServerEvent('wdadgeagteafacetefaxc:setJobVehicleState', data2.current.vehicleProps.plate, false)
...
function OpenShopMenu(elements, restoreCoords, shopCoords)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.Game.SpawnLocalVehicle(data.current.model, shopCoords, 250.37, function(vehicle)
	...
	ESX.Game.SpawnLocalVehicle(elements[1].model, shopCoords, 250.37, function(vehicle)
		table.insert(spawnedVehicles, vehicle)
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
	end)
...
RegisterNetEvent('scarface_fickdeinemuttedwwadwdawdadwadwwrduskid:wafanfwopfdwadwdawwfpw')
AddEventHandler('scarface_fickdeinemuttedwwadwdawdadwadwwrduskid:wafanfwopfdwadwdawwfpw', function(healType, quiet)
	local playerPed = PlayerPedId()
