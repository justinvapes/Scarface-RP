...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelCoords = coords
	AppelID = id
	ESX.ShowAdvancedNotification("LSMC", "~b~LSMC Zentrale", "Jemand benötigt einen Medic! Betätige deine konfigurierten Buttons, um den Auftrag an- oder abzulehnen.", "CHAR_CALL911", 8)
end)
...
Citizen.CreateThread(function()
     while true do
		...
		if IsControlJustPressed(1, 246) and AppelDejaPris == true then
			ESX.ShowAdvancedNotification("LSMC", "~b~LSMC Zentrale", "Der Auftrag wurde bereits von einem anderen Medic angenommen.", "CHAR_CALL911", 8)
		end
     end
end)

RegisterCommand('ambulanceaccept',function()
...
RegisterCommand('ambulancedenied',function ()
	if AppelEnAttente then
		if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'ambulance' then
			ESX.ShowAdvancedNotification("LSMC", "~b~LSMC Zentrale", "Du hast den Auftrag abgelehnt", "CHAR_CALL911", 8)
...
RegisterKeyMapping('ambulanceaccept','Auftrag Annehmen','keyboard','h')
RegisterKeyMapping('ambulancedenied','Auftrag Ablehenen','keyboard','x')
...
AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
	ESX.ShowAdvancedNotification("LSMC", "~b~LSMC Zentrale", "Du hast den Auftrag angenommen, folge der Markierung auf dem GPS.", "CHAR_CALL911", 8)   
...
function afficherTextVolant(XAcoords, XAid)
	...
	AddTextComponentSubstringPlayerName("~b~Alarmiere LSMC")
	...
	-- Ajout de la route
     ...
     while rea do
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
               ...
               Draw3DText(XAcoords.x, XAcoords.y, XAcoords.z, "~g~Hier ist eine Person, die wiederbelebt werden muss ...\n~b~[E]~w~um zu wiederleben.", 4, 0.1, 0.1)
               ...
               if IsControlJustReleased(0, 38) then
				...
				TriggerServerEvent('scarface_fickdeinemutterduskid:wafanfwopfwfpw', XAid)
...
Citizen.CreateThread( function()	
	while true do
		...
		if enService then
			...
			DrawAdvancedText(0.984, 0.214, 0.008, 0.0028, 0.4, "Letzten Aufträge:", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.988, 0.236, 0.005, 0.0028, 0.4, "~b~"..NomAppel.." ~w~hat den letzten Auftrag angenommen.", 255, 255, 255, 255, 6, 0)
			DrawAdvancedText(0.984, 0.274, 0.008, 0.0028, 0.4, "Insgesammt Anzahl an angenommen Aufrägen.", 0, 191, 255, 255, 6, 0)
