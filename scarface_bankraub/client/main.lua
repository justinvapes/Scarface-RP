...
RegisterNetEvent('scarface_bankraub:currentlyRobbing')
AddEventHandler('scarface_bankraub:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('scarface_bankraub:killBlip')
AddEventHandler('scarface_bankraub:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('scarface_bankraub:setBlip')
AddEventHandler('scarface_bankraub:setBlip', function(position)
...
RegisterNetEvent('scarface_bankraub:tooFar')
AddEventHandler('scarface_bankraub:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('scarface_bankraub:robberyComplete')
AddEventHandler('scarface_bankraub:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('scarface_bankraub:startTimer')
AddEventHandler('scarface_bankraub:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('scarface_bankraub:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('scarface_bankraub:tooFar', store)
			end
		end
	end
end)
