	...
	TriggerEvent('esx_skin:openRestrictedMenu', function(data, menu)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data.current.value == 'yes' then
				ESX.TriggerServerCallback('esx_chainshop:buyClothes', function(bought)
					if bought then
						...
						ESX.TriggerServerCallback('esx_chainshop:checkPropertyDataStore', function(foundStore)
							if foundStore then
								ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'save_dressing', {
									...
									if data2.current.value == 'yes' then
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'outfit_name', {
											...
											TriggerEvent('skinchanger:getSkin', function(skin)
												TriggerServerEvent('esx_chainshop:saveOutfit', data3.value, skin)
...
AddEventHandler('esx_chainshop:hasEnteredMarker', function(zone)
	currentAction     = 'shop_menu'
	currentActionMsg  = _U('press_menu')
	currentActionData = {}
end)

AddEventHandler('esx_chainshop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not hasAlreadyEnteredMarker) or (isInMarker and lastZone ~= currentZone) then
			...
			TriggerEvent('esx_chainshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_chainshop:hasExitedMarker', lastZone)
		end
