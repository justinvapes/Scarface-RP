...
RegisterNetEvent('scarface_yachtraub:currentlyRobbing')
AddEventHandler('scarface_yachtraub:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('scarface_yachtraub:killBlip')
AddEventHandler('scarface_yachtraub:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('scarface_yachtraub:setBlip')
AddEventHandler('scarface_yachtraub:setBlip', function(position)
...
RegisterNetEvent('scarface_yachtraub:tooFar')
AddEventHandler('scarface_yachtraub:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('scarface_yachtraub:robberyComplete')
AddEventHandler('scarface_yachtraub:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('scarface_yachtraub:startTimer')
AddEventHandler('scarface_yachtraub:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('scarface_yachtraub:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('scarface_yachtraub:tooFar', store)
			end
		end
	end
end)
