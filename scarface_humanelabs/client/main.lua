...
RegisterNetEvent('scarface_humanelabs:currentlyRobbing')
AddEventHandler('scarface_humanelabs:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('scarface_humanelabs:killBlip')
AddEventHandler('scarface_humanelabs:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('scarface_humanelabs:setBlip')
AddEventHandler('scarface_humanelabs:setBlip', function(position)
...
RegisterNetEvent('scarface_humanelabs:tooFar')
AddEventHandler('scarface_humanelabs:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('scarface_humanelabs:robberyComplete')
AddEventHandler('scarface_humanelabs:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('scarface_humanelabs:startTimer')
AddEventHandler('scarface_humanelabs:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('scarface_humanelabs:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('scarface_humanelabs:tooFar', store)
			end
		end
	end
end)
