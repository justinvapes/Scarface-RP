RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" or PlayerData.job.name == "lssd" or PlayerData.job.name == "fib" or PlayerData.job.name == "army" then
		OpenJailMenu()
	else
		ESX.ShowNotification("Du bist nicht berechtigt dazu!")
	end
end)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Security" then
				...
				if PlayerData.job.name ~= "police" or PlayerData.job.name ~= "lssd" or PlayerData.job.name ~= "fib" or PlayerData.job.name ~= "army" then
					ESX.ShowNotification("Du hast keine Berechtigung dazu das SG zu verlassen")
					return
				end
			end
...
Citizen.CreateThread(function()
    ...
    AddTextComponentString('Staatsgefängnis')
