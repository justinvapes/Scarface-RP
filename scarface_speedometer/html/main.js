	...
	window.addEventListener('message', function(event) {
		...
		if (event.data.action == "setTalking"){
            ...
            if(event.data.status){
            	 document.getElementById("Ellipse_2_y").style.fill="#f80a0f";
            }else{
            	document.getElementById("Ellipse_2_y").style.fill="#202020";
            }
		} else if (event.data.action == "lockSwitch"){
            if(event.data.status){
            	document.getElementById("Ellipse_2_").style.fill="#202020";
            }else{
            	document.getElementById("Ellipse_2_").style.fill="#f80a0f";
            }
		}else if (event.data.action == "toggleAllHud"){
		...
		}else if (event.data.action == "tempomat"){
			if (event.data.show){
				document.getElementById("speed").style.color = '#f80a0f';
				document.getElementById("KMH").style.color = '#f80a0f';
			}else{
				document.getElementById("speed").style.color = '#fff';
				document.getElementById("KMH").style.color = '#fff';
			}
...
function setProgressSpeed(value, element){
  ...
  function setProgressFuel(percent, element){
    ...
    document.getElementById('fueltext').innerHTML = percent ;
  }
