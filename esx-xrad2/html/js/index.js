    ...
    window.addEventListener("message", function (event) {
		...
		$(document).keyup(function (e) {
			if (e.key === "Escape") {
				...
				$.post('http:///esx-xrad2/escape', JSON.stringify({}));
			}
		});
    });
});
		...
		$(document).keyup(function (e) {
			if (e.keyCode === 88) {
				if(typeb == "0"){
				$.post('http:///esx-xrad2/ichhabgarnixgemacht', JSON.stringify({}));
				$('body').css('display', 'none');
				typeb = "0"
				}
				if(typeb == "1"){
				...
				if(typeb == "2"){
				$.post('http:///esx-xrad2/Autoaufundzu', JSON.stringify({}));
				typeb = "0"
				}
				if(typeb == "3"){
				$.post('http:///esx-xrad2/Motor', JSON.stringify({}));
				typeb = "0"
				}
				if(typeb == "4"){
				$.post('http:///esx-xrad2/Radio', JSON.stringify({}));
				typeb = "0"
				}
			}
		});
