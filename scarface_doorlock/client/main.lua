...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('scarface_doorlock:getDoorInfo', function(doorInfo)
			...
			if distance < maxDistance then
				...
				if IsControlJustReleased(0, 38) then
					if isAuthorized then
						...
						if doorID.locked then
						TriggerEvent('scarface_notify', "#ff0000", "Information", "Abgeschlossen")
						TriggerServerEvent('scarface_doorlock:updateState', k, doorID.locked)
					else
						TriggerEvent('scarface_notify', "#3bfd00", "Information", "Aufgeschlossen")
						TriggerServerEvent('scarface_doorlock:updateState', k, doorID.locked) -- Broadcast new state of the door to everyone
					end
					end
				end
			end
...
RegisterNetEvent('scarface_doorlock:setState')
AddEventHandler('scarface_doorlock:setState', function(doorID, state)
	Config.DoorList[doorID].locked = state
end)